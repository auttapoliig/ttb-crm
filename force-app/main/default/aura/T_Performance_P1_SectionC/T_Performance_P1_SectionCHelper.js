({
    ...
    loadTabs: function (cmp, event) {
        ...
        var currSelected = cmp.get('v.selectedPeriod');
        if(currSelected != tab.get('v.id')) {
            cmp.set('v.selectedPeriod',tab.get('v.id'))
            if(tab.get('v.id') == 'month') {
                var lastAvailDataTimeObj = cmp.get('v.lastAvailDataTimeObj');
                if(cmp.get('v.selectedYear') == cmp.get('v.currentYear')) {
                    cmp.set('v.selectedMonth',this.getMonthInit(lastAvailDataTimeObj.currYearDefaultMonth))
                } else {
                    cmp.set('v.selectedMonth',this.getMonthInit(lastAvailDataTimeObj.prevYearDefaultMonth))
                }
            }
    ...
    getMonthInit : function(monthNo) {
        var monthLst = [
            { init: 'Jan', month: 'January' },
            { init: 'Feb', month: 'February' },
            { init: 'Mar', month: 'March' },
            { init: 'Apr', month: 'April' },
            { init: 'May', month: 'May' },
            { init: 'Jun', month: 'June' },
            { init: 'Jul', month: 'July' },
            { init: 'Aug', month: 'August' },
            { init: 'Sep', month: 'September' },
            { init: 'Oct', month: 'October' },
            { init: 'Nov', month: 'November' },
            { init: 'Dec', month: 'December' },
        ];

        if(monthNo && monthNo <= monthLst.length) {
            return monthLst[monthNo-1].init;
        }
        return null;
    },

    drawChart : function(component, event, helper) {
        var kpi = component.get('v.financePerf.individualPerf');
      ...
      var el2 = component.find("chartNeedle").getElement();
        ...
        // console.log('debug needle value',chartObj.config.data)

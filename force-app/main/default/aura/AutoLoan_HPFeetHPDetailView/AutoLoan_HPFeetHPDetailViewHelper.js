({
    ...
    callProduct: function (component, event, helper, round) {
        ...
        var action = component.get('c.getHPFeetDetail');
        action.setParams({
        ...
        action.setCallback(this, function (response) {
            ...
            if (component.isValid() && state === 'SUCCESS') {
                ...
                if(result.HTTPStatusCode == '401' && round < $A.get("$Label.c.Number_Of_Retry_Times")){
                    setTimeout(() => {
                        helper.callProduct(component, event, helper, round+1);
                    }, $A.get("$Label.c.Retry_SetTimeOut"));
                }
                else{
                    ...
                    if(result.Authorize == true){
                        ...
                        helper.genHiddenData(component, helper);
                    }
    ...
    genHiddenData : function(component, helper){
        var hpDatas = component.get('v.fields.HpInformation');
        var guarantorDatas = component.get('v.fields.Guarantor');

        hpDatas.forEach(item => {
            item.value = $A.get('$Label.c.Data_Condition_Hidden_Text');
        });
        guarantorDatas.forEach(item => {
            item.value = $A.get('$Label.c.Data_Condition_Hidden_Text');
        });
